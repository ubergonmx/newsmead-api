...
train_news_file = os.path.join(tmp_dir, "train", r"news.tsv")
train_behaviors_file = os.path.join(tmp_dir, "train", r"behaviors.tsv")
valid_news_file = os.path.join(tmp_dir, "valid", r"news.tsv")
valid_behaviors_file = os.path.join(tmp_dir, "valid", r"behaviors.tsv")
wordEmb_file = os.path.join(tmp_dir, "utils", "embedding_all.npy")
userDict_file = os.path.join(tmp_dir, "utils", "uid2index.pkl")
wordDict_file = os.path.join(tmp_dir, "utils", "word_dict_all.pkl")
vertDict_file = os.path.join(tmp_dir, "utils", "vert_dict.pkl")
subvertDict_file = os.path.join(tmp_dir, "utils", "subvert_dict.pkl")
yaml_file = os.path.join(tmp_dir, "utils", r"naml.yaml")
...
if not os.path.exists(train_news_file):
    download_deeprec_resources(
        mind_url, os.path.join(tmp_dir, "train"), mind_train_dataset
    )

if not os.path.exists(valid_news_file):
    download_deeprec_resources(
        mind_url, os.path.join(tmp_dir, "valid"), mind_dev_dataset
    )
if not os.path.exists(yaml_file):
    download_deeprec_resources(
        r"https://recodatasets.z20.web.core.windows.net/newsrec/",
        os.path.join(tmp_dir, "utils"),
        mind_utils,
    )
...
res_path = os.path.join(tmp_dir, "utils", "evaluation_results.txt")
